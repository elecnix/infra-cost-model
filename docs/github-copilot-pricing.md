...
- **`infra_cost_model/pricing/github_copilot.py`**: authoritative plan and
  per-model rate data, plus `estimate_credits()` (tokens → credits) and
  `compute_copilot_cost()` (plan + seats + consumption → breakdown).
